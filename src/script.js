...
if (window._guiInitialized) {
    ...
    // Fragment Shader - ONLY modify the specific lines with errors
    const fragmentShader = `
      ...
      uniform float largeElementSpeedRatio;
      uniform float smallElementSpeedRatio;
      ...
      // Add a pseudo-random function based on a seed value
      float random(vec2 st) {
        return fract(sin(dot(st.xy, vec2(12.9898, 78.233))) * 43758.5453123);
      }
      
      // Add fractal noise function for more natural randomness
      ...
      void main() {
        ...
        vec2 flowPrimary1 = vec2(
          sin(animTime1 * 0.25 + seed * 1.1) * 0.6 + cos(animTime1 * 0.13) * 0.2,
          cos(animTime1 * 0.3 + seed * 0.7) * 0.6 + sin(animTime1 * 0.21) * 0.1
        );
        
        vec2 flowPrimary2 = vec2(
          cos(animTime1 * 0.15 - seed * 1.3) * 0.7 + sin(animTime1 * 0.19) * 0.15, 
          -sin(animTime1 * 0.2 - seed * 0.9) * 0.7 + cos(animTime1 * 0.11) * 0.2
        );
        
        // Secondary flow directions - NORMAL SPEED
        vec2 flowDir1 = vec2(
          sin(animTime2 * 0.5 + seed * 1.4 + sin(chaosTime * 0.02) * 0.3) * 0.5,
          -cos(animTime2 * 0.45 + seed * 0.6 + cos(chaosTime * 0.03) * 0.2) * 0.5
        );
        
        vec2 flowDir2 = vec2(
          -cos(animTime2 * 0.4 - seed * 1.25) * 0.6 + sin(animTime2 * 0.17) * 0.2, 
          sin(animTime2 * 0.55 - seed * 0.35) * 0.4 + cos(animTime2 * 0.23) * 0.15
        );
        
        vec2 flowDir3 = vec2(
          sin(animTime2 * 0.6 + seed * 1.2 + cos(chaosTime * 0.04) * 0.25) * 0.4,
          cos(animTime2 * 0.35 + seed * 0.45 + sin(chaosTime * 0.05) * 0.3) * 0.6
        );
        
        // Cross-flowing currents - SLOWER
        vec2 crossFlow1 = vec2(
          cos(animTime3 * 0.37 + seed * 0.92) * 0.5 + sin(chaosTime * 0.07) * 0.2, 
          sin(animTime3 * 0.28 + seed * 1.53) * 0.5 + cos(chaosTime * 0.06) * 0.1
        );
        
        vec2 crossFlow2 = vec2(
          -sin(animTime3 * 0.41 - seed * 1.31) * 0.4 + cos(chaosTime * 0.08) * 0.15,
          cos(animTime3 * 0.33 - seed * 0.42) * 0.6 + sin(chaosTime * 0.09) * 0.1
        );
        
        // Slow waves with long periods - SUPER FAST
        vec2 slowWaveDir1 = vec2(
          sin(animTime1 * 0.08 + seed * 0.82) * 0.9 + cos(animTime1 * 0.05) * 0.1,
          cos(animTime1 * 0.07 + seed * 0.71) * 0.8 + sin(animTime1 * 0.04) * 0.15
        );
        
        vec2 slowWaveDir2 = vec2(
          cos(animTime1 * 0.06 - seed * 0.95) * 0.8 + sin(animTime1 * 0.05) * 0.15,
          sin(animTime1 * 0.09 - seed * 0.75) * 0.9 + cos(animTime1 * 0.04) * 0.1
        );
        
        // Fast erratic directions - VERY SLOW
        vec2 fastErraticDir1 = vec2(
          sin(animTime3 * 1.2 + cos(chaosTime * 0.7) * 0.6) * 0.3,
          cos(animTime3 * 1.4 + sin(chaosTime * 0.9) * 0.7) * 0.3
        );
        
        vec2 fastErraticDir2 = vec2(
          cos(animTime3 * 1.6 - sin(chaosTime * 1.1) * 0.5) * 0.35, 
          sin(animTime3 * 1.3 - cos(chaosTime * 0.8) * 0.6) * 0.25
        );
        
        // Non-uniform rotation effects - SLOW
        ...
        float largeSlowWave1 = fractalNoise(
          vec2(
            ...
            uv.y * 0.5 * varyingComplexity + slowWaveDir1.y * animTime1 * 0.04
          ), 
          complexity, 
          seed + animTime1 * 0.002, 
          3
        ) * 0.6;
        
        float largeSlowWave2 = fractalNoise(
          vec2(
            uv.x * 0.3 * varyingComplexity + slowWaveDir2.x * animTime1 * 0.03,
            uv.y * 0.6 * varyingComplexity + slowWaveDir2.y * animTime1 * 0.03
          ), 
          complexity, 
          seed + 2.76 + animTime1 * 0.003, 
          4
        ) * 0.45;
        ...
        // Small fast erratic movements with animTime3 (small elements)
        float erraticIntensity = 0.15 + pow(sin(animTime2 * 1.23) * 0.5 + 0.5, 4.0) * 0.2;
        float smallFastNoise1 = fractalNoise(
          vec2(
            uv.x * 12.0 * varyingComplexity + fastErraticDir1.x * animTime3 * 3.0, // Increased multiplier for more obvious effect
            uv.y * 10.0 * varyingComplexity + fastErraticDir1.y * animTime3 * 3.0  // Increased multiplier for more obvious effect
          ),
          complexity,
          seed + 10.34 + animTime3 * 0.05, // Use animTime3 in the seed too for more variation
          2
        ) * erraticIntensity;
        
        float smallFastNoise2 = fractalNoise(
          vec2(
            rotatedUV1.x * 14.0 * varyingComplexity + fastErraticDir2.x * animTime3 * 2.4,
        ...
        float colorTransition1 = sin(animTime1 * 0.6) * 0.5 + 0.5;
        float colorTransition2 = sin(animTime2 * 0.42 + 0.5) * 0.5 + 0.5;
        float colorTransition3 = sin(animTime3 * 0.5 + 1.3) * 0.5 + 0.5; // Increased coefficient for more obvious effect
        ...
        colorTransition = mix(
          colorTransition,
          sin(time * 0.2 + uv.x + uv.y) * 0.5 + 0.5,
          turbulence * 0.3
        );
        
        // Generate color noise patterns with controlled speed
        float colorNoise1 = snoise(vec2(
          uv.x * 1.0 * complexity + flowDir1.x * animTime1 * 0.2 + seed * 0.07,
          uv.y * 1.0 * complexity + flowDir1.y * animTime2 * 0.2 + seed * 0.07
        )) * 0.5 + 0.5;
        
        float colorNoise2 = snoise(vec2(
          uv.x * 1.5 * complexity + flowDir2.x * animTime1 * 0.15 - seed * 0.05,
          uv.y * 1.5 * complexity + flowDir2.y * animTime1 * 0.15 - seed * 0.05
        )) * 0.5 + 0.5;
        
        float colorNoise3 = snoise(vec2(
          uv.x * 2.0 * complexity + flowDir3.x * animTime2 * 0.1 + seed * 0.03,
          uv.y * 2.0 * complexity + flowDir3.y * animTime2 * 0.1 + seed * 0.03
        )) * 0.5 + 0.5;
        
        float colorNoise4 = snoise(vec2(
          uv.x * 2.5 * complexity + flowDir1.y * animTime3 * 0.15 - seed * 0.04,
          uv.y * 2.5 * complexity + flowDir2.x * animTime3 * 0.15 - seed * 0.04
        )) * 0.5 + 0.5;
        
        float colorNoise5 = snoise(vec2(
          uv.x * 3.0 * complexity + sin(animTime3 * 0.1) * 0.1 + seed * 0.05,
          uv.y * 3.0 * complexity + cos(animTime3 * 0.1) * 0.1 + seed * 0.05
        )) * 0.5 + 0.5;
        ...
        float timeBasedMix = sin(animTime1 * 0.2 + seed) * 0.2 + 0.5;
        ...
        // For grain patterns - use static values based on seed (no animation)
        float fineGrain = fractalNoise(
          uv * 800.0 + vec2(seed * 0.1, seed * 0.2), // Use seed for position instead of animated direction
          1.0, 
          seed + 16.78, // removed chaosTime dependency
          2
        ) * 0.04 * grainAmount;

        // Medium grain with static positioning
        float mediumGrain = fractalNoise(
          rotatedUV1 * 400.0 + vec2(seed * 0.3, seed * 0.4), // Use seed for position instead of animated direction
          1.0, 
          seed + 17.91, // removed chaosTime dependency
          2
        ) * 0.03 * grainAmount;
        
        // Create a completely static grain mask
        float grainMask = 1.0; // completely static mask
    ...
    const material = new THREE.ShaderMaterial({
        ...
        uniforms: {
            ...
            largeElementSpeedRatio: { value: settings.largeElementSpeedRatio },
            smallElementSpeedRatio: { value: settings.smallElementSpeedRatio },
    ...
    animationFolder.add(settings, 'smallElementSpeedRatio', 0.05, 2.0, 0.05)
        .name('Small Element Speed')
        .onChange(value => material.uniforms.smallElementSpeedRatio.value = value);
    ...
    function exportCurrentSettings() {
      try {
        ...
        const currentSettings = {
          ...
          largeElementSpeedRatio: settings.largeElementSpeedRatio,
          smallElementSpeedRatio: settings.smallElementSpeedRatio
        };
  ...
  <script>
    ...
    const material = new THREE.ShaderMaterial({
        ...
        uniforms: {
            ...
            largeElementSpeedRatio: { value: \${currentSettings.largeElementSpeedRatio} },
            smallElementSpeedRatio: { value: \${currentSettings.smallElementSpeedRatio} },
